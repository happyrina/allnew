...
app.get("/update", async (req, res) => {
  const { ST_ID, NAME, DEPT } = req.query
  const [rows, fields] = await pool.query("UPDATE st_info SET NAME=?, DEPT=? WHERE ST_ID=?", [NAME, DEPT, ST_ID]);
...
app.get("/delete", async (req, res) => {
  ...
  const [results] = await pool.query("DELETE FROM st_info WHERE ST_ID=?", [ST_ID]);
